...
const Projects = () => {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const res = await axios.get(`${prod_be_url}/projects`, {
        headers: {
      ...
      if (res.data?.success) {
        ...
        showErrorToast(res.data.message || 'Failed to fetch projects');
      }
    } catch (error) {
      showErrorToast('An error occurred while fetching projects');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative">
      ...
      <div className={`p-8 text-black bg-gray-50 min-h-screen ${loading ? 'opacity-30 pointer-events-none' : ''}`}>
        ...
        <p className="text-lg text-gray-700 mb-4">
          Total Projects: <span className="font-semibold text-indigo-700">{projects.length}</span>
        </p>
          ...
          <table className="min-w-full bg-white border border-gray-300 shadow-md rounded-lg">
            <thead className="bg-gradient-to-r from-indigo-500 to-purple-500 text-white">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm font-semibold border">Key</th>
                <th className="px-6 py-3 text-left text-sm font-semibold border">Name</th>
                <th className="px-6 py-3 text-left text-sm font-semibold border">Project Type</th>
              </tr>
            </thead>
            <tbody className="text-sm text-black divide-y divide-gray-200">
              {projects.map((item, index) => (
                <tr key={item.ID} className="hover:bg-gray-100 bg-indigo-50">
                  ...
                  <td className="px-4 py-4 border">{item.Key}</td>
                  <td className="px-6 py-4 border">{item.Name}</td>
                  <td className="px-6 py-4 border capitalize">{item["Project Type"]}</td>
                </tr>
              ))}
            </tbody>
          </table>
